...
open class JXSegmentedTitleCell: JXSegmentedBaseCell {
    ...
    public var titleLabelCenterX = NSLayoutConstraint()
    public var titleLabelCenterY = NSLayoutConstraint()
    public var maskTitleLabelCenterX = NSLayoutConstraint()
    public var maskTitleLabelCenterY = NSLayoutConstraint()
    ...
    open override func commonInit() {
        ...
        titleLabelCenterX = titleLabel.centerXAnchor.constraint(equalTo: contentView.centerXAnchor, constant: 0)
        titleLabelCenterY = titleLabel.centerYAnchor.constraint(equalTo: contentView.centerYAnchor, constant: 0)

        maskTitleLabel.textAlignment = .center
        maskTitleLabel.isHidden = true
        maskTitleLabel.translatesAutoresizingMaskIntoConstraints = false
        contentView.addSubview(maskTitleLabel)
        
        maskTitleLabelCenterX = maskTitleLabel.centerXAnchor.constraint(equalTo: contentView.centerXAnchor)
        maskTitleLabelCenterY = maskTitleLabel.centerYAnchor.constraint(equalTo: contentView.centerYAnchor)
        ...
        NSLayoutConstraint.activate([titleLabelCenterX, titleLabelCenterY, maskTitleLabelCenterX, maskTitleLabelCenterY])
    }

    open override func layoutSubviews() {
        super.layoutSubviews()

        guard let myCellModel = itemModel as? JXSegmentedTitleItemModel else { return }
        
        let titleH = NSString(string: myCellModel.title ?? "").boundingRect(with: CGSize(width: CGFloat.infinity, height: CGFloat.infinity), options: [.usesFontLeading, .usesLineFragmentOrigin], attributes: [NSAttributedString.Key.font : myCellModel.titleNormalFont], context: nil).size.height
        
        switch myCellModel.titleLabelAnchorPointStyle {
        case .center:
            titleLabel.layer.anchorPoint = CGPoint(x: 0.5, y: 0.5)
            maskTitleLabel.layer.anchorPoint = CGPoint(x: 0.5, y: 0.5)
            titleLabelCenterY.constant = 0 + myCellModel.titleLabelVerticalOffset
            break
        case .top:
            titleLabel.layer.anchorPoint = CGPoint(x: 0.5, y: 0)
            maskTitleLabel.layer.anchorPoint = CGPoint(x: 0.5, y: 0)
            let percent = (myCellModel.titleCurrentZoomScale - myCellModel.titleNormalZoomScale) / (myCellModel.titleSelectedZoomScale - myCellModel.titleNormalZoomScale)
            titleLabelCenterY.constant = -titleH/2-myCellModel.titleLabelVerticalOffset - myCellModel.titleZoomSelectedVerticalOffset*percent
            break
        case .bottom:
            titleLabel.layer.anchorPoint = CGPoint(x: 0.5, y: 1)
            maskTitleLabel.layer.anchorPoint = CGPoint(x: 0.5, y: 1)
            let percent = (myCellModel.titleCurrentZoomScale - myCellModel.titleNormalZoomScale)/(myCellModel.titleSelectedZoomScale - myCellModel.titleNormalZoomScale)
            titleLabelCenterY.constant = titleH/2 + myCellModel.titleLabelVerticalOffset + myCellModel.titleZoomSelectedVerticalOffset*percent
            break
        }
...
//        maskTitleLabel.center = contentView.center
    ...
    open override func reloadData(itemModel: JXSegmentedBaseItemModel, selectedType: JXSegmentedViewItemSelectedType) {
        ...
        accessibilityLabel = myItemModel.title
    ...
    override func setSelectedStyle(isSelected: Bool) {
        if isSelected {
            titleLabel.textColor = (itemModel as? JXSegmentedTitleItemModel)?.titleSelectedColor
        }else {
            titleLabel.textColor = (itemModel as? JXSegmentedTitleItemModel)?.titleNormalColor
        }
    }
}
